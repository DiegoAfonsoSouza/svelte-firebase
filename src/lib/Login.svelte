<script>
    import { authSignIn, authSignInGoogle } from "$lib/auth";

    let emailInput = '';
    let passwordInput = '';
    
    async function realizarLogin() {
       await authSignIn(emailInput, passwordInput);
    }
    ...
    <div class="container flex items-center justify-center min-h-screen px-6 mx-auto">
        <form class="w-full max-w-md" on:submit|preventDefault={realizarLogin}>
            ...
            <div class="mt-6">
                ...
                <p class="mt-4 text-center text-gray-600 dark:text-gray-400">ou entre com</p>

                <button type="button" on:click={authSignInGoogle} class="flex w-full items-center justify-center px-6 py-3 mt-4 text-gray-600 transition-colors duration-300 transform border rounded-lg dark:border-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                <div class="mt-6 text-center ">
                    <a href="#" class="text-sm text-blue-500 hover:underline dark:text-blue-400">
                        Não tem uma conta ainda? Inscrever-se
                    </a>
                </div>
            </div>
        </form>
